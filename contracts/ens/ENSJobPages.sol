...
contract ENSJobPages is Ownable, ERC1155Holder {
    ...
    // Keep enough headroom so any valid ENS label (<=63) can be concatenated as <label>.<root>
    // without exceeding the DNS/ENS full-name limit (253 bytes).
    uint256 private constant MAX_ROOT_NAME_LENGTH = 189;
