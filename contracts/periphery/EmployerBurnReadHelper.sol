...
contract EmployerBurnReadHelper {
        ...
        returns (uint256 reserveAmount, bool reserveFunded, uint256 lockedReserveTotal)
    {
        ...
        bool terminal = completed || expired;
        reserveFunded = expected == 0 || (!terminal && lockedReserveTotal >= expected);
    }
    ...
    {
        (, uint256 payout, bool completed, bool disputed, bool expired) = _readCore(jobId);
        uint256 expected = (payout * manager.employerBurnBps()) / 10_000;
        reserveFunded = expected == 0 || manager.lockedBurnReserves() >= expected;
        if (completed || expired) return (false, false, false, READINESS_ALREADY_TERMINAL, 0);
        if (manager.settlementPaused()) return (false, reserveFunded, false, READINESS_SETTLEMENT_PAUSED, 0);
