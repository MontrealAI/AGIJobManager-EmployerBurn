...
Source fingerprint: d77e173c3a84ecb4
...
- `function _initRoots(bytes32[4] memory rootNodes, bytes32[2] memory merkleRoots) internal` ([contracts/AGIJobManager.sol#L607](../../contracts/AGIJobManager.sol#L607))
- `function lockIdentityConfiguration() external onlyOwner whenIdentityConfigurable` ([contracts/AGIJobManager.sol#L765](../../contracts/AGIJobManager.sol#L765))
- `function applyForJob(uint256 _jobId, string memory subdomain, bytes32[] calldata proof)` ([contracts/AGIJobManager.sol#L814](../../contracts/AGIJobManager.sol#L814))
- `function validateJob(uint256 _jobId, string memory subdomain, bytes32[] calldata proof)` ([contracts/AGIJobManager.sol#L876](../../contracts/AGIJobManager.sol#L876))
- `function disapproveJob(uint256 _jobId, string memory subdomain, bytes32[] calldata proof)` ([contracts/AGIJobManager.sol#L884](../../contracts/AGIJobManager.sol#L884))
...
- `function lockJobENS(uint256 jobId, bool burnFuses) external` ([contracts/AGIJobManager.sol#L1358](../../contracts/AGIJobManager.sol#L1358))
- `function tokenURI(uint256 tokenId) public view override returns (string memory)` ([contracts/AGIJobManager.sol#L1582](../../contracts/AGIJobManager.sol#L1582))
- `function _callEnsJobPagesHook(uint8 hook, uint256 jobId) internal` ([contracts/AGIJobManager.sol#L1587](../../contracts/AGIJobManager.sol#L1587))
...
- `event EnsRegistryUpdated(address newEnsRegistry);` ([contracts/AGIJobManager.sol#L494](../../contracts/AGIJobManager.sol#L494))
- `event RootNodesUpdated(` ([contracts/AGIJobManager.sol#L496](../../contracts/AGIJobManager.sol#L496))
- `event MerkleRootsUpdated(bytes32 validatorMerkleRoot, bytes32 agentMerkleRoot);` ([contracts/AGIJobManager.sol#L502](../../contracts/AGIJobManager.sol#L502))
- `event IdentityConfigurationLocked(address indexed locker, uint256 indexed atTimestamp);` ([contracts/AGIJobManager.sol#L509](../../contracts/AGIJobManager.sol#L509))
- `event EnsJobPagesUpdated(address indexed oldEnsJobPages, address indexed newEnsJobPages);` ([contracts/AGIJobManager.sol#L516](../../contracts/AGIJobManager.sol#L516))
- `event EnsHookAttempted(uint8 indexed hook, uint256 indexed jobId, address indexed target, bool success);` ([contracts/AGIJobManager.sol#L532](../../contracts/AGIJobManager.sol#L532))
...
- @notice Anyone may lock ENS records after a job reaches a terminal state; only the owner may burn fuses. ([contracts/AGIJobManager.sol#L1356](../../contracts/AGIJobManager.sol#L1356))
- @dev Fuse burning is irreversible and remains owner-only; ENS hook execution is best-effort. ([contracts/AGIJobManager.sol#L1357](../../contracts/AGIJobManager.sol#L1357))
- @dev as long as lockedEscrow/locked*Bonds are fully covered. ([contracts/AGIJobManager.sol#L1404](../../contracts/AGIJobManager.sol#L1404))
- @dev Owner withdrawals are limited to balances not backing lockedEscrow/locked*Bonds. ([contracts/AGIJobManager.sol#L1617](../../contracts/AGIJobManager.sol#L1617))
