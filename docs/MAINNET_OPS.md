...
- **Employer-burn mode (corrected successor)**: burn is charged only in `createJob` via `burnFrom(employer, burnAmount)` where `burnAmount = payout * employerBurnBps / 10_000`.
...
6. If `employerBurnBps > 0`, run createJob smoke and confirm `EmployerBurnChargedAtJobCreation` emits and token total supply decreases by expected amount at posting time.
