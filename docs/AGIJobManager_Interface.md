...
| --- | --- | --- | --- | --- |
| `createJob(string _jobSpecURI, uint256 _payout, uint256 _duration, string _details)` | Employer | `whenNotPaused`; payout/duration within limits; URI is non‑empty & no whitespace; employer has sufficient balance/allowance for payout + createJob burn | Atomically transfers payout escrow into manager and burns createJob-only AGIALPHA posting cost from employer wallet; stores snapshots; emits `EmployerBurnChargedAtJobCreation` and `JobCreated`. | `InvalidParameters`, `TransferFailed`, `Pausable: paused`. |
...
| `updateAGITokenAddress(address _newTokenAddress)` | Anyone | None (function is intentionally disabled) | Compatibility stub only; always reverts because successor release is AGIALPHA-pinned. | `AGIALPHATokenPinned`. |
...
| `withdrawAGI(uint256 amount)` | Owner | amount > 0; ≤ withdrawable | Transfers surplus ERC‑20; emits `AGIWithdrawn`. | `InvalidParameters`, `InsolventEscrowBalance`, `InsufficientWithdrawableBalance`, `TransferFailed`. |
