...
- **Source-of-truth deploy/verify artifact:** `hardhat/artifacts/**` generated by this Hardhat project.
- **Foundry (forge) role: audit/fuzz testing only** (non-canonical for production deployment artifacts).
- **Truffle role: legacy compatibility only** for historical reproducibility.
