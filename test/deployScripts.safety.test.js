...
contract('Hardhat deploy scripts safety checks', () => {
  ...
  it('deploy-ens-job-pages.js requires explicit mainnet JOB_MANAGER/JOBS_ROOT_NAME/JOBS_ROOT_NODE', () => {
    assert(ensDeployScript.includes('Mainnet deploy requires explicit ${key} environment variable (no implicit default on chainId=1).'));
    assert(ensDeployScript.includes('requireExplicitMainnetEnv("JOB_MANAGER")'));
    assert(ensDeployScript.includes('requireExplicitMainnetEnv("JOBS_ROOT_NAME")'));
    assert(ensDeployScript.includes('requireExplicitMainnetEnv("JOBS_ROOT_NODE")'));
  });
  ...
  it('deploy-ens-job-pages.js rejects root name/root node mismatch and prints manual cutover steps', () => {
    ...
    assert(ensDeployScript.includes('JOBS_ROOT_NAME must already be ENS-normalized.'));
