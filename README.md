...
AGIJobManager EmployerBurn v0.2.0 is an Ethereum smart-contract system for escrowed AGI work agreements where AGIALPHA burn is funded only by the Employer and executed only on successful completion settlement, with optional ENS-backed job pages managed by `ENSJobPages`.
Legacy identity string for tooling compatibility: **Employer-burn semantics (this repo variant)**.

> [!WARNING]
> **Deprecation notice (semantic correction):** the prior release line (v0.1.x) enforced burn on employer-refund/employer-win paths. That behavior is semantically wrong for completion-only burn requirements. Use v0.2.0 successor deployments for production.
...
- **Employer-burn semantics (v0.2.0):** employers pre-fund a dedicated burn reserve at job creation; reserve burns only on successful completion paths and is refunded on employer-refund/cancel/expiry/non-success outcomes. No protocol treasury subsidy is used.

### Employer-funded completion-burn quick note (Etherscan-first)

- Mainnet AGIALPHA token: `0xA61a3B3a130a9c20768EEBF97E21515A6046a1fA`.
- Burn executes only on successful completion settlement (`_completeJob`), including successful dispute completion outcomes.
- Burn does **not** execute on employer-refund, cancellation, expiry, tie/under-quorum forced-dispute, unresolved, or employer-win dispute outcomes.
...
- Employers fund payout escrow + burn reserve up front in a single create-job approval/transfer.
- Burn observability: settlement emits `EmployerBurnEnforced(jobId, employer, token, amount, finalizer, settlementPathCode)` when non-zero completion burn is applied.
